...
type sqlitedb from n_sqlite within w_test_sqlite
end type
end forward
...
if IsFailed(sqlitedb.Update(dw_1,true,false)) then
	sErrText = sqlitedb.SQLErrText()
	sqlitedb.Rollback()
	MessageBox("UPDATE",sErrText)
	return
end if

if IsFailed(sqlitedb.Commit()) then
	sErrText = sqlitedb.SQLErrText()
	sqlitedb.Rollback()
...
if IsFailed(sqlitedb.Query(dw_1/*[,sql]*/)) then
	MessageBox("SELECT",sqlitedb.SQLErrText())
	return
end if
...
//检索到DataStore
if IsFailed(sqlitedb.Query(ref ds,"SELECT * FROM COMPANY")) then
	MessageBox("SELECT",sqlitedb.SQLErrText())
	return
end if
...
if IsFailed(sqlitedb.Update(ds)) then
	sErrText = sqlitedb.SQLErrText()
	sqlitedb.Rollback()
	MessageBox("UPDATE",sErrText)
	return
...
event clicked;sqlitedb.SetAutoCommit(true)
if IsFailed(sqlitedb.Exec("UPDATE COMPANY SET SALARY = SALARY + 100 WHERE NAME = ? ",&
							"Paul")) then
	MessageBox("UPDATE",sqlitedb.SQLErrText())
end if
...
//检索到Recordset
if IsFailed(sqlitedb.Query(ref rs,"SELECT * FROM COMPANY")) then
	MessageBox("SELECT",sqlitedb.SQLErrText())
	return
end if
...
//批量插入
if IsFailed(sqlitedb.Exec("INSERT INTO COMPANY (NAME,AGE,ADDRESS,SALARY,BIRTH) "  	+&
...
event clicked;if sqlitedb.IsOpened() then return
...
sqlitedb.SetAutoCommit(true)

if IsFailed(sqlitedb.Exec("CREATE TABLE IF NOT EXISTS COMPANY(" 	+&
							...
							"SALARY         REAL," 						+&
							"BIRTH          TEXT)")) then
	MessageBox("CREATE TABLE",sqlitedb.SQLErrText())
end if
...
type sqlitedb from n_sqlite within w_test_sqlite descriptor "pb_nvo" = "true" 
end type
...
event ondberror;call super::ondberror;MessageBox("DBError","code: " + String(code) + ", error: " + sqlErrorText + ", sql: " + sqlSyntax)
end event
