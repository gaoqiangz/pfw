$PBExportHeader$s_cst_editmask.sru
$PBExportComments$PowerFramework~r~n(c)飞沃共享(深圳)科技有限公司
forward
global type s_cst_editmask from editmask
end type
type icondata from structure within s_cst_editmask
end type
type theme from n_cst_singlelineedit_theme within s_cst_editmask
end type
end forward
...
global s_cst_editmask s_cst_editmask
...
private subroutine _of_adjustrect (ref rectf adjrect, readonly unsignedlong flags);if Border then
	if BitAND(flags,ADJ_BORDER) > 0 then
		if theme.#BorderStyle = Enums.BS_SOLID or theme.#BorderStyle = Enums.BS_ROUND then
			Painter.InflateRect(ref adjRect,-1,-1)
...
on s_cst_editmask.create
this.theme=create theme
end on

on s_cst_editmask.destroy
destroy(this.theme)
end on

type theme from n_cst_singlelineedit_theme within s_cst_editmask descriptor "pb_nvo" = "true" 
end type
