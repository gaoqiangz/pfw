$PBExportHeader$s_cst_hprogressbar.sru
$PBExportComments$PowerFramework~r~n(c)飞沃共享(深圳)科技有限公司
forward
global type s_cst_hprogressbar from hprogressbar
end type
type theme from n_cst_progressbar_theme within s_cst_hprogressbar
end type
type timer from n_timer within s_cst_hprogressbar
end type
end forward
...
global s_cst_hprogressbar s_cst_hprogressbar
...
privatewrite	string	#Text				//显示的文本
...
if #Animation or #Marquee or SmoothScroll then
	_bAnimating = true
...
ulong clrTransparent,clrFkgnd,hRgn,nDTParm
real fRight
...
if clrTransparent <> 0 then
	Painter.FillRect(hdc,rcClient.left,rcClient.top,rcClient.right,rcClient.bottom,clrTransparent,clrTransparent,Enums.VERT,false,false,0)
end if

Painter.ThemeFill(hdc,rcClient,theme.of_GetColor(theme.CLR_BKGND,0),Enums.VERT,theme.#BkgndStyle,0,false,theme.#BorderStyle,0,theme.#RoundSize)

fRight = rcClient.right
...
if Not #Marquee and Int(Position) >= Int(MinPosition) and MaxPosition > MinPosition then
	...
	rcClient.right = (nPos / (MaxPosition - MinPosition)) * rcClient.right
	Painter.ThemeFill(hdc,rcClient,clrFkgnd,Enums.VERT,theme.#BkgndStyle,0,false,theme.#BorderStyle,0,theme.#RoundSize)
end if
...
if ((Position < MaxPosition or _nSlidePos <> 0) and #Animation) or #Marquee then
	...
	if theme.#BkgndStyle = Enums.SOLID or #Marquee then
		rcSlide.right = Min(rcClient.right,40)
	else
		rcSlide.right = Min(Max(rcClient.right,40),60)
	end if
	rcSlide.bottom = rcClient.bottom
	Painter.OffsetRect(ref rcSlide,(_nSlidePos / SLIDE_MAX_POS) * rcClient.right - rcSlide.right,0)
	if theme.#BkgndStyle = Enums.SOLID or #Marquee then
		if Not #Marquee then
			clrFkgnd = ARGBLighten(clrFkgnd,0.3)
		end if
		Painter.FillRect(hdc,rcSlide.left,rcSlide.top,rcSlide.right,rcSlide.bottom,0,clrFkgnd,Enums.HORZ,false,false,0)
	else
		if Not #Marquee then
	...
	if rcSlide.left >= rcClient.right then
		_nSlidePos = 0
		_nSlideTicks = CPU()
	end if
end if

rcClient.right = fRight

if #Text <> "" then
	nDTParm = Painter.DT_SINGLELINE + Painter.DT_VCENTER + Painter.DT_WORD_ELLIPSIS
	if theme.#TextAlign = Left! then
		nDTParm += Painter.DT_LEFT
	elseif theme.#TextAlign = Center! then
		nDTParm += Painter.DT_CENTER
	elseif theme.#TextAlign = Right! then
		nDTParm += Painter.DT_RIGHT
	end if
	Painter.DrawText(hdc,theme.Font.#Handle,#Text,rcClient,theme.of_GetColor(theme.CLR_TEXT,0),false,nDTParm)
end if
...
on s_cst_hprogressbar.create
this.theme=create theme
this.timer=create timer
end on

on s_cst_hprogressbar.destroy
destroy(this.theme)
destroy(this.timer)
end on

type theme from n_cst_progressbar_theme within s_cst_hprogressbar descriptor "pb_nvo" = "true" 
end type
...
type timer from n_timer within s_cst_hprogressbar descriptor "pb_nvo" = "true" 
end type
