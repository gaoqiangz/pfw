$PBExportHeader$s_cst_checkbox.sru
$PBExportComments$PowerFramework~r~n(c)飞沃共享(深圳)科技有限公司
forward
global type s_cst_checkbox from checkbox
end type
type theme from n_cst_checkbox_theme within s_cst_checkbox
end type
end forward
...
global s_cst_checkbox s_cst_checkbox
...
if Checked then
	if ThirdState then
		...
		elseif _Focused then
			__ImageList.Draw(__Idx_CBSC_Focused,hdc,&
											rcImage.left,&
											rcImage.top,&
											Not Enabled)
		else
		...
		elseif _MouseOver then
			__ImageList.Draw(__Idx_CBC_Hover,hdc,&
											...
											Not Enabled)
		elseif _Focused then
			__ImageList.Draw(__Idx_CBC_Focused,hdc,&
											rcImage.left,&
											rcImage.top,&
											Not Enabled)
		else
	...
	elseif _MouseOver then
		__ImageList.Draw(__Idx_CB_Hover,hdc,&
										...
										Not Enabled)
	elseif _Focused then
		__ImageList.Draw(__Idx_CB_Focused,hdc,&
										rcImage.left,&
										rcImage.top,&
										Not Enabled)
	else
...
event type long onlbuttondown(unsignedlong vkey, real xpos, real ypos);long rtCode
...
event type long onsetcheck(long ncheck);of_Redraw(false)
return 0
end event
...
on s_cst_checkbox.create
this.theme=create theme
end on

on s_cst_checkbox.destroy
destroy(this.theme)
end on

type theme from n_cst_checkbox_theme within s_cst_checkbox descriptor "pb_nvo" = "true" 
end type
