...
export class pfwSidebar extends Element {
    ...
    _handleLClick(target, dbl) {
        ...
        if (elTarget.$('>options')) {
            if (elTarget.parentElement !== this || !this.state.collapsed) {
                ...
                else this.expandOption(elTarget, true);
            }
        } else {
    ...
    _buildVIcon(image) {
        ...
        if (iconfont.isUri(image)) {
            ...
            return (
                <span class={`sb-img ${clsFont}`} state-html={charCode}></span>
            );
        } else {
            let sStyle = '';
            let i = image.indexOf('{');
    ...
    _buildVOption(text, textIsHtml, image, tipText) {
        ...
        tipText = tipText || '';
        return (
            <option text={!textIsHtml ? text : undefined} image={image}>
    ...
    selectOption(elOption, triggerEvent = false) {
        ...
        if (this._elSelected) this._elSelected.state.current = true;
        if (triggerEvent) {
    ...
    setText(elOption, text, textIsHtml = false) {
        if (!elOption) return false;
        if (textIsHtml) elOption.$('>caption>text').innerHTML = text;
        else elOption.$('>caption>text').innerText = text;
