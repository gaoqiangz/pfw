...
export class pfwTreeView extends Element {
    ...
    _buildVIcon(image) {
        ...
        if (iconfont.isUri(image)) {
            ...
            return (
                <span class={`tv-img ${clsFont}`} state-html={charCode}></span>
            );
        } else {
            let sStyle = '';
            let i = image.indexOf('{');
    ...
    _buildVOption(
        ...
        return (
            <option
                text={!textIsHtml ? text : undefined}
                image={image}
                class={disabledLook ? 'disable' : undefined}
            >
                <caption
                    imgpos={image ? imagePosition : undefined}
                    tooltip={tipText}
                >
                    {['left', 'top'].includes(imagePosition)
                        ? this._buildVIcon(image)
                        : undefined}
                    <text state-html={textIsHtml ? text : undefined}>
                        {!textIsHtml ? text : undefined}
                    </text>
    ...
    load(data) {
        ...
        const loader = (options, data) => {
            for (let item of data) {
                ...
                if (item.checked) vopt[1]['state-checked'] = true;
                if (item.children instanceof Array) {
                    if (item.checked) vopt[1]['state-collapsed'] = true; //使其具有[:node]状态
    ...
    insertOption(
        ...
        let elOption = this._buildVOption(
            text,
            textIsHtml,
            image,
            imagePosition,
        ...
        switch (place) {
            case PLACE_BEFORE: {
                ...
                elOwner.parentElement.insertBefore(
                    elOwner.parentElement.lastElementChild,
                    elOwner
                );
            ...
            case PLACE_AFTER: {
                ...
                elOwner.parentElement.append(elOption);
                elOwner.parentElement.insertAfter(
                    elOwner.parentElement.lastElementChild,
                    elOwner
                );
    ...
    getCheckState(elOption) {
        ...
        return elOption.state.incomplete
            ? CHECK_STATE_INDETERMINATE
            : elOption.state.checked
            ? CHECK_STATE_CHECKED
    ...
    setText(elOption, text, textIsHtml) {
        ...
        if (textIsHtml) elText.innerText = text;
    ...
    editText(elOption, newText) {
        ...
        elInput.value =
            typeof newText === 'string' ? newText : elOption.attributes['text'];
        ...
        elInput._acceptText = function () {
            ...
            let rejected = tree._sendEvent(EVT_END_TEXT_EDIT, {
                target: elOption,
                reason: reason,
                newText: this.value,
            });
        ...
        elInput.onkeydown = function (evt) {
            if (evt.code === 'Enter') {
                ...
                tree._sendEvent(EVT_END_TEXT_EDIT, {
                    target: elOption,
                    reason: 2,
                });
                elText.innerText = elOption.attributes['text'];
        ...
        if (image) {
            ...
            if (elImg) {
                ...
                if (['left', 'top'].includes(imgpos)) {
                    elCaption.insertBefore(
                        elCaption.lastElementChild,
                        elCaption.firstElementChild
                    );
                }
    ...
    setDisabledLook(elOption, set) {
        if (!elOption) return false;
        set
            ? elOption.classList.add('disable')
            : elOption.classList.remove('disable');
        return true;
    }
    ...
    selectOption(elOption, fireEvent) {
        if (!elOption || !elOption.$is('option') || elOption.state.disabled)
            return false;
        let elCurrent = this._elSelected;
        if (elCurrent === elOption) return true;
        if (fireEvent)
            if (
                this._sendEvent(EVT_SELECTION_CHANGING, {
                    old: elCurrent,
                    new: elOption,
                })
            )
        ...
        if (fireEvent)
            this._sendEvent(EVT_SELECTION_CHANGED, {
                old: elCurrent,
                new: elOption,
            });
    ...
    expandOption(elOption, fireEvent) {
        ...
        if (fireEvent) this._sendEvent(EVT_EXPANDED, elOption);
        return true;
    }
    ...
    expandParentOption(elOption, fireEvent) {
        ...
        while (elParent) {
            if (!elParent.state.expanded) {
                ...
                if (fireEvent) this._sendEvent(EVT_EXPANDED, elParent);
            }
            elParent = elParent.parentElement
                ? elParent.parentElement.$p('option')
                : null;
        }
        return true;
    }
    ...
    collapseOption(elOption, fireEvent) {
        ...
        if (fireEvent) this._sendEvent(EVT_COLLAPSED, elOption);
        return true;
    }
    ...
    checkOption(elOption, fireEvent) {
        ...
        if (fireEvent) {
            function chk(el) {
                ...
                if (
                    this._sendEvent(EVT_CHECK_STATE_CHANGING, {
                        target: el,
                        old: oldState,
                        new: newState,
                    })
                ...
                if (newState === CHECK_STATE_CHECKED) {
                    if (!el.state.checked) {
                        ...
                        this._sendEvent(EVT_CHECK_STATE_CHANGED, {
                            target: el,
                            old: oldState,
                            new: newState,
                        });
        ...
        if (fireEvent) {
            function unchk(el) {
                ...
                if (this.isNode(el)) {
                    ...
                    for (let elChild of elChildren) {
                        if (
                            !elChild.state.checked &&
                            !elChild.state.incomplete
                        ) {
                            ++nUnchkCnt;
                            continue;
                        }
    ...
    removeOption(elOption, changeSelection, fireEvent) {
        ...
        if (elOption === this._elSelected && changeSelection) {
            ...
            if (elOption === this._elSelected) {
                ...
                while (elSelect) {
                    if (this.selectOption(elSelect.$p('option'), fireEvent))
                        break;
...
class EventHandler {
    ...
    hitTest(evt, elOption, where) {
        ...
        switch (where) {
            case HIT_CAPTION: {
                const [x1, y1, x2, y2] = elOption
                    .$('>caption')
                    .state.box('rect', 'border', 'window', false);
                return (
                    evt.windowX >= x1 &&
                    evt.windowX <= x2 &&
                    evt.windowY >= y1 &&
    ...
    onMouseDown(evt, opt) {
        ...
        if (this.hitTest(evt, opt, HIT_COLLAPSE_BUTTON)) {
            if (opt.state.expanded) this.tree.collapseOption(opt, true);
            else if (opt.state.collapsed) this.tree.expandOption(opt, true);
        } else if (
            !this.hitTest(evt, opt, HIT_CHECKBOX) &&
            this.hitTest(evt, opt, HIT_CAPTION)
        ) {
    ...
    onMouseUp(evt, opt) {
        if (evt.target.$is('input')) return false;
        if (!opt) return false;
        if (this._lastMouseEvent !== 'mousedown' || this._lastMouseOpt !== opt)
            return false;
        if (evt.button === 1) {
    ...
    onKeyDown(evt, opt) {
        ...
        switch (evt.code) {
            ...
            case 'ArrowRight': {
                if (this.tree.isNode(opt)) {
                    if (opt.state.collapsed) this.tree.expandOption(opt, true);
                    else this.tree.selectOption(opt.$('>option'), true);
                }
                return true;
            }
            case 'ArrowUp': {
            ...
            case 'ArrowDown': {
                let optNext;
                if (opt.state.expanded) optNext = opt.$('>option');
                else optNext = opt.nextElementSibling;
                ...
                if (!optNext) {
                    ...
                    while (optParent && optParent !== this) {
                        if (
                            optParent.nextElementSibling &&
                            !optParent.nextElementSibling.state.disabled
                        ) {
            ...
            case 'Space': {
                if (!opt.state.checked) this.tree.checkOption(opt, true);
                else this.tree.uncheckOption(opt, true);
                return true;
            }
        }
...
class DragDropHandler {
    ...
    constructor(tree) {
        this.tree = tree;
        this.px_as_dip =
            tree.state.pixelsIn('1px') === tree.state.pixelsIn('1dip');
    ...
    onMouse(evt) {
        ...
        switch (evt.type) {
            case 'mousedown': {
                ...
                const [x, y, w, h] = this.getElementLocalRectW(
                    opt.$('>caption')
                );
            ...
            case 'mousemove': {
                ...
                if (this._dragging) {
                    ...
                    const [evtX, evtY] = this.getEventLocalPos(evt);
                    if (
                        Math.abs(evtX - this._mouse_down_pos.x) >
                            DD_X_THRESHOLD ||
    ...
    doBeginDrag(evt) {
        ...
        if (this.tree._sendEvent(EVT_BEGIN_DRAG, this._mouse_down_opt))
            return false;
    ...
    doDragMove(evt) {
        ...
        if (elFound && this.belongsTo(elFound, this._drag_source)) {
            elFound = null;
        }
        if (elFound && !elFound.$is('option')) elFound = elFound.$p('option');
        if (
            elFound &&
            (elFound === this._drag_source || elFound.state.disabled)
        ) {
            elFound = null;
        }
        ...
        if (this._mouse_target !== elFound) {
            ...
            if (this._drop_target) {
                this.tree._sendEvent(EVT_DRAG_LEAVE, {
                    drag: this._drag_source,
                    target: this._drop_target,
                    where: this._drop_where,
                });
                this._drop_target.classList.remove('dragenter');
            ...
            if (nWhere !== this._mouse_where) {
                ...
                switch (nWhere) {
                    case DD_DROP_WHERE_BEFORE: {
                        if (
                            this._drag_source ===
                            this._mouse_target.previousElementSibling
                        ) {
                            nWhere = 0;
                        }
                        break;
                    ...
                    case DD_DROP_WHERE_AFTER: {
                        if (
                            this._drag_source ===
                            this._mouse_target.nextElementSibling
                        ) {
                            nWhere = 0;
                        }
                        break;
                    ...
                    case DD_DROP_WHERE_INSIDE: {
                        if (
                            this._drag_source.parentElement ===
                            this._mouse_target
                        ) {
                            nWhere = 0;
                        }
                ...
                if (nWhere !== 0) {
                    if (
                        this.tree._sendEvent(EVT_DRAG_ENTER, {
                            drag: this._drag_source,
                            target: this._mouse_target,
                            where: nWhere,
                        })
                    ) {
                        nWhere = 0;
                ...
                if (this._drop_where !== 0) {
                    [x, y, w, h] = this.getElementLocalRectW(
                        this._mouse_target
                    );
    ...
    doDrop(evt) {
        ...
        if (this._drop_target && evt.button === 1) {
            if (
                !this.tree._sendEvent(EVT_DRAG_DROP, {
                    drag: this._drag_source,
                    target: this._drop_target,
                    where: this._drop_where,
                })
            ) {
                const elParent = this._drag_source.parentElement;

                if (this.tree.isCheckBoxEnabled()) {
                    this.tree.refreshParentCheckState(
                        this._drag_source,
                        false,
                        true
                    );
                ...
                switch (this._drop_where) {
                    case DD_DROP_WHERE_BEFORE: {
                        if (
                            this._drag_source !==
                            this._drop_target.previousElementSibling
                        ) {
                            this._drop_target.parentElement.insertBefore(
                                this._drag_source,
                                this._drop_target
                            );
                            //FIXME 强制刷新布局
                    ...
                    case DD_DROP_WHERE_AFTER: {
                        if (
                            this._drag_source !==
                            this._drop_target.nextElementSibling
                        ) {
                            this._drop_target.parentElement.insertAfter(
                                this._drag_source,
                                this._drop_target
                            );
                            //FIXME 强制刷新布局
    ...
    onPaintFkgnd(gfx) {
        ...
        gfx.draw(this._snap_image, {
            ...
            width: this._snap_image_pos.w,
            height: this._snap_image_pos.h,
            opacity: 0.5,
        });
        if (this._drop_target) {
            ...
            if (this._drop_where === DD_DROP_WHERE_INSIDE) {
                ...
                gfx.beginPath();
                gfx.rect(
                    this._drop_marker_pos.x1,
                    this._drop_marker_pos.y1,
                    this._drop_marker_pos.x2,
                    this._drop_marker_pos.y2
                );
                gfx.fill();
                gfx.stroke();
            } else {
