...
pfw为Blink导出了PB接口对象,使JS脚本可以无缝与PB交互,在使用之前请仔细阅读以下内容说明

## 导出的全局对象(类似`window`)

- `pb`: PB入口对象
- `pb.global`: PB全局变量和全局函数访问对象
- `pb.host`: PB端的Blink宿主对象

> pfw为每个Blink窗口(控件)定义了一个`pb.host`对象属性,该对象为当前Blink宿主PB对象的引用,如:`u_blink`,`n_blink`,`w_blink`
